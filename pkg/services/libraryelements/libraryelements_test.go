...
func createDashboard(t *testing.T, sqlStore *sqlstore.SQLStore, user models.SignedInUser, dash *models.Dashboard, folderID int64) *models.Dashboard {
	...
	dashboard, err := dashboardservice.ProvideDashboardService(dashboardStore, dashAlertExtractor, starFake).SaveDashboard(context.Background(), dashItem, true)
	require.NoError(t, err)
...
func testScenario(t *testing.T, desc string, fn func(t *testing.T, sc scenarioContext)) {
	...
	t.Run(desc, func(t *testing.T) {
		...
		dashboardStore := database.ProvideDashboardStore(sqlStore)
		starFake := startest.NewStarServiceFake()
		dashboardService := dashboardservice.ProvideDashboardService(dashboardStore, &alerting.DashAlertExtractorService{}, starFake)
		service := LibraryElementService{
			Cfg:           setting.NewCfg(),
			SQLStore:      sqlStore,
